...
tax_table <- as.data.frame(data.obj.rff$otu.tab) #rarified data
...
#write to csv for Supplementary Dataset
#tax_table_g_raw <- as.data.frame(data.obj$abund.list$Genus)
#dim(tax_table_g_raw)
#write.csv(tax_table_g_raw, file = "tax_table_g_raw.csv")
...
genus_names <- as.character(sapply(rownames(tax_table_g_RA_prev), function(x) strsplit(x, ";")[[1]][2]))
rownames(tax_table_g_RA_prev) <- genus_names
...
#OTUdata <- t(tax_table_g_RA_prev) + 0.05; #relative abundance data scaled to 100% 
...
names(cluster_list) <- c("misc1", "Faecalibacterium\nDorea", 
                         "Blautia\nStreptoccus", "misc2", "Bacteroidales")
...
png("./Figure/RM_figures/heatmap fastCCLasso MCCM revision.png",width = 14.5*300, height=12*300, res=300)
set.seed(42)
heatmap_plot_fastCCLasso
dev.off()
...
rownames(tax_table_g_RA) <- all_genus_names
...
write.csv(res_clus_df_fastCCLasso_2, "./Figure/RM_figures/res_clus_df_fastCCLasso_revision.csv", row.names = T)
...
#misc1                   -3.8995328 7.481874e-14 1.496375e-13
#Faecalibacterium\nDorea -7.1014889 5.943254e-23 2.381933e-22
#Blautia\nStreptoccus    10.3344340 7.939776e-23 2.381933e-22
#misc2                    3.1672373 2.392594e-06 3.588891e-06
#Bacteroidales           -0.9884386 1.860003e-01 1.860003e-01
...
#ordered from most enriched to most depleted in Cancer
col_ordered <- c("Blautia\nStreptoccus", "misc2", "Bacteroidales", "other", "misc1", "Faecalibacterium\nDorea")
...
png(paste0("./Figure/RM_figures/boxplots_fastCCLasso_cancer_vs_healthy revision.png"), width = 6.2*300, height=6*300, res = 300)
...
png(paste0("./Figure/RM_figures/boxplots_fastCCLasso_cancer_vs_healthy revision_only3.png"), width = 6.2*300, height=4.5*300, res = 300)
...
png(paste0("./Figure/RM_figures/fastCClasso_assocation_with_cancer_type revision.png"), width = 6.25*300, height=7*300, res = 300)
hm$heatmap_plot
dev.off()


#----------------------------------------
#now plot only the 3 named co-abundance groups


fdr_df <- fdr_df[,col_ordered_sub]
estimate_df <- estimate_df[,col_ordered_sub]
...
png(paste0("./Figure/RM_figures/fastCClasso_assocation_with_cancer_type revision_only3.png"), width = 5.5*300, height=7*300, res = 300)
hm$heatmap_plot
dev.off()
...
res_clus_df_fastCCLasso_sort <- res_clus_df_fastCCLasso[order(as.numeric(res_clus_df_fastCCLasso$`Blautia
Streptoccus`), decreasing = T),]
...
par(mfrow=c(5,1))
par(mar=c(1,1,1,1))
plot(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$`Blautia
Streptoccus`, col=sel_colors[1], lwd=2, axes = F)
points(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$misc1, col=sel_colors[2], lwd=2)
legend("topright", colnames(res_clus_df_fastCCLasso_sort)[1:2], fill = sel_colors[c(1,2)], border="white", bty = "n")

plot(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$`Blautia
Streptoccus`, col=sel_colors[1], lwd=2, axes = F)
points(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$`Faecalibacterium
Dorea`, col=sel_colors[3], lwd=2)
legend("topright", colnames(res_clus_df_fastCCLasso_sort)[c(1,3)], fill = sel_colors[c(1,3)], border="white", bty = "n")

plot(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$`Blautia
Streptoccus`, col=sel_colors[1], lwd=2, axes = F)
points(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$misc2, col=sel_colors[4], lwd=2)
legend("topright", colnames(res_clus_df_fastCCLasso_sort)[c(1,4)], fill = sel_colors[c(1,4)], border="white", bty = "n")

plot(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$`Blautia
Streptoccus`, col=sel_colors[1], lwd=2, axes = F)
points(1:nrow(res_clus_df_fastCCLasso_sort), res_clus_df_fastCCLasso_sort$Bacteroidales, col=sel_colors[5], lwd=2)
